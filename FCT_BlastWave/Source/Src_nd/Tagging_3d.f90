...
subroutine state_error(tag,tag_lo,tag_hi, &
                       ...
                       maxgradpx,maxgradpy,maxgradpz,tagfrac,rad_bw, &
  ...
  real(amrex_real) :: problo(3), dx(3), time, maxgradpx, maxgradpy, maxgradpz, tagfrac, rad_bw
  integer          :: set,clear

  integer          :: i, j, k
  real(amrex_real) :: dro, gradpx, gradpy, gradpz
  ...
  !$omp parallel do private(i,j,k,gradpx,gradpy,gradpz) collapse(3)
    do       k = lo(3), hi(3)
      do    j = lo(2), hi(2)
        do i = lo(1), hi(1)
            ...
            gradpz = 0.5_amrex_real*(state(i,j,k+1,pre) - state(i,j,k-1,pre))/dx(3)
            if(abs(gradpx) > tagfrac*maxgradpx .or. &
            &  abs(gradpy) > tagfrac*maxgradpy .or. &
            &  abs(gradpz) > tagfrac*maxgradpz) then
              tag(i,j,k) = set
            endif
        enddo
      enddo
