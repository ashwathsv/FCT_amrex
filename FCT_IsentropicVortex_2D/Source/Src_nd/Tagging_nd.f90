...
subroutine state_error(tag,tag_lo,tag_hi, &
                       ...
                       set,clear,&
                       lo,hi,&
                       dx,problo,time,nc,rofs,tagfrac) bind(C, name="state_error")
  ...
  real(amrex_real) :: problo(3), dx(3), time, rofs, tagfrac
  integer          :: set,clear

  integer          :: i, j, k, rank
  real(amrex_real) :: dro, xcm, ycm, mid(2)
  ...
  do       k = lo(3), hi(3)
     do    j = lo(2), hi(2)
        do i = lo(1), hi(1)
              dro = state(i,j,k,ro) - rofs
              if(abs(dro) > tagfrac*rofs) then
                tag(i,j,k) = set
              endif
        enddo
     enddo
